...
Definir una base profesional de auditoria operativa para el backend UENS, con estos objetivos:

1. Trazabilidad de operaciones críticas en tiempo real.
2. Evidencia técnica para revisión administrativa y academica.
...
3. Es necesario demostrar por que un reporte se genero, fallo o fue reintentado.

Por eso se implementa una auditoria operativa simple pero realista:
...
1. Bajo acoplamiento: auditoria se registra en su propia tabla (`auditoria_evento`).
2. Tolerancia a falla: si no se puede auditar, no se cae la operación funcional.
3. Evidencia útil: actor, requestId, IP, módulo, accion, resultado y detalle.
4. Seguridad por rol: solo ADMIN consulta eventos y solicita reporte de auditoria.
5. Reuso de infraestructura: el reporte de auditoria usa la misma DB queue de reportes.

---

## 4. Criterios de auditoria (que se registra y como)
...
- `accion`: accion concreta (`SOLICITUD_CREADA`, `WORKER_SOLICITUD_ERROR`, etc.).
- `entidad` y `entidad_id`: objeto afectado.
- `resultado`: `EXITO | ERROR | INFO | ADVERTENCIA`.
- `detalle`: contexto corto de negocio/técnico.
- `request_id`: correlacion entre logs, API y auditoria.
...
3. Mantener mensajes cortos y estables para busqueda/filtros.
...
- `ReporteSolicitudCommandService`: creacion de solicitud de auditoria (solo ADMIN) + trazas.
- `ReporteSolicitudWorkerService`: traza de exito/error por procesamiento.
- `AuditoriaAdminOperacionesProcessor`: payload del reporte de auditoria.
...
Resultado: al resetear o reseedear, no quedan residuos de auditoria/reportes anteriores.
...
## 7.2 Solicitud de reporte de auditoria

- Metodo: `POST /reportes/solicitudes`
- Rol: `ADMIN`
...
## 8. Flujo de procesamiento del reporte de auditoria

1. ADMIN solicita reporte por endpoint de auditoria.
...
1. API: `@PreAuthorize("hasRole('ADMIN')")` en endpoints de auditoria.
2. Aplicacion: validación adicional en `ReporteSolicitudCommandService#crearSolicitudAuditoria`.
...
3. El módulo auditoria no depende de frontend ni de formato visual especifico.
4. El reporte de auditoria es un consumidor adicional del pipeline ya estable.
...
1. Riesgo: auditoria no debe bloquear negocio.
...
3. Solicitar reporte de auditoria con rango válido -> `201` + estado `PENDIENTE`.
...
6. Forzar error de processor y confirmar evento `WORKER_SOLICITUD_ERROR` en auditoria.

---

## 14. Proximos pasos recomendados

1. Exportar reporte de auditoria con plantilla dedicada por formato (tabla + resumen + metadata).
...
4. Integrar dashboard de auditoria (KPIs de errores, volumen por módulo, top acciones).
